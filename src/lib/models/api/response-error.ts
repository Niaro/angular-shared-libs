...
export class ResponseError {
	...
	constructor(e: HttpErrorResponse) {
		this.status =
			e.status >= 500 || e.statusText === 'Unknown Error'
		...
		if (this.status === StatusCode.notFound) return this;

		if (this.status === StatusCode.internalServerError)
			this.messages = [
				{
					message:
						'We are sorry! The server has encountered an internal error and was unable to complete your request.',
