...
export abstract class Enumeration {
	...
	constructor(valueOrDisplayName?: number | string, displayName?: string) {
		// do not access the {name} property in the constructor because it is lazy initialized and required all static variables
		...
		if (isNumber(valueOrDisplayName)) {
			...
			// Schedule the microtask at the event of current event loop
			// which means that the constructor will have all the enumerations attached to it
			// and we are able to find by the id of the enum its name in amidst the static properties
			Promise.resolve().then(() => {
				this._value = this.name = this.getValueName();
				this.cssClass = this.getCssClass();
			});
		}
	}
